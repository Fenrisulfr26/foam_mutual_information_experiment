...
state.latestRows = 16;
state.latestCols = 16;
state.latestImageMode = 'sum';
state.latestSmoothCurves = false;
state.latestSmoothWindow = 5;
...
%% --- UI construction ---
    function handles = buildUi()
        ...
        uicontrol(handles.controlPanel, 'Style', 'text', ...
            'Units', 'pixels', ...
            'Position', [18, 10, 70, 22], ...
            'HorizontalAlignment', 'left', ...
            'String', 'Intensity');

        handles.imageModePopup = uicontrol(handles.controlPanel, 'Style', 'popupmenu', ...
            'Units', 'pixels', ...
            'Position', [90, 10, 125, 26], ...
            'String', {'Total', 'Peak'}, ...
            'Value', 1, ...
            'Callback', @onPlotOverview);

        handles.smoothCheck = uicontrol(handles.controlPanel, 'Style', 'checkbox', ...
            'Units', 'pixels', ...
            'Position', [235, 10, 115, 24], ...
            'String', 'Smooth curves', ...
            'Value', 0, ...
            'Callback', @onPlotOverview);

        uicontrol(handles.controlPanel, 'Style', 'text', ...
            'Units', 'pixels', ...
            'Position', [365, 10, 35, 22], ...
            'HorizontalAlignment', 'left', ...
            'String', 'Win');

        handles.smoothWindowEdit = uicontrol(handles.controlPanel, 'Style', 'edit', ...
            'Units', 'pixels', ...
            'Position', [400, 10, 55, 26], ...
            'HorizontalAlignment', 'left', ...
            'String', '5', ...
            'Callback', @onPlotOverview);
...
%% --- UI callbacks ---
    ...
    function onShowIntensityOverview(~, ~)
        try
            ...
            [smoothCurves, smoothWindow] = getSmoothSettings();
            ...
            for pointIdx = 1:9
                ...
                histCubeForDisplay = prepareHistForDisplay(histCube, smoothCurves, smoothWindow);
                intensityImage = makeIntensityImage(histCubeForDisplay, imageMode);
            ...
            fig = figure( ...
                'Name', sprintf('3x3 %s intensity images | %s', imageMode, upper(modeName)), ...
                'NumberTitle', 'off', ...
                'Color', 'w');
            ...
            layout = tiledlayout(fig, 3, 3, ...
                'TileSpacing', 'compact', ...
                'Padding', 'compact');
            title(layout, sprintf('3x3 %s intensity images | %s', imageMode, upper(modeName)), ...
                'Interpreter', 'none');
    ...
    function onPlotOverview(~, ~)
        try
            ...
            state.latestImageMode = imageMode;
            state.latestSmoothCurves = smoothCurves;
            state.latestSmoothWindow = smoothWindow;
            ...
            for pointIdx = 1:9
                ...
                axis(ax, 'on');
                histCubeForDisplay = sanitizeAndSmoothHist(histCube, smoothCurves, smoothWindow);
                curve = sumSelectedCurve(histCubeForDisplay, rowIdx, colIdx);
...
%% --- Data loading / plotting ---
    ...
    function imageMode = getSelectedImageMode()
        value = get(handles.imageModePopup, 'Value');
        if value == 2
            imageMode = 'peak';
        else
            imageMode = 'sum';
    ...
    function [smoothCurves, smoothWindow] = getSmoothSettings()
        smoothCurves = logical(get(handles.smoothCheck, 'Value'));
        smoothWindow = str2double(get(handles.smoothWindowEdit, 'String'));

        if ~isfinite(smoothWindow) || smoothWindow < 1
            smoothWindow = 5;
        end
        ...
        set(handles.smoothWindowEdit, 'String', num2str(smoothWindow));
    end
    ...
    function histCubeForDisplay = prepareHistForDisplay(histCube, smoothCurves, smoothWindow)
        histCubeForDisplay = correct_hot_dark_pixels(histCube);
        histCubeForDisplay = sanitizeAndSmoothHist(histCubeForDisplay, smoothCurves, smoothWindow);
    end

    function histCubeForDisplay = sanitizeAndSmoothHist(histCube, smoothCurves, smoothWindow)
        histCubeForDisplay = double(histCube);
        histCubeForDisplay(~isfinite(histCubeForDisplay)) = 0;

        if smoothCurves && smoothWindow > 1
            histCubeForDisplay = movmean(histCubeForDisplay, smoothWindow, 3);
        end
    end

    function intensityImage = makeIntensityImage(histCubeForDisplay, imageMode)
        if strcmpi(imageMode, 'peak')
            intensityImage = max(histCubeForDisplay, [], 3);
        else
            intensityImage = sum(histCubeForDisplay, 3);
        end
    end
    ...
    function openFullPointView(pointIdx, modeName)
        try
            ...
            imageMode = getSelectedImageMode();
            [smoothCurves, smoothWindow] = getSmoothSettings();
            histCubeForDisplay = prepareHistForDisplay(histCube, false, smoothWindow);

            displayOptions = struct();
            displayOptions.imageMode = imageMode;
            displayOptions.smoothCurves = smoothCurves;
            displayOptions.smoothWindow = smoothWindow;

            my_display_hist(histCubeForDisplay, displayOptions);
            fig = gcf;
